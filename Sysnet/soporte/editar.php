...
if (!isset($_POST["accion"])) {
	...
	//levanto datos de la solicitud a editar
	$sql = "SELECT * FROM soporte WHERE soporte_id = '$id'";
	...
	if (!$resultado["soporte_id"]) {
		...
		$estado_id = $resultado["soporte_estado_id"];
		$fecha_solicitud = $resultado["soporte_fecha_solicitud"]; 
		$caso = $resultado["soporte_nro_caso"];
		$descripcion = $resultado["soporte_descripcion"];
		$reclamo = $resultado["soporte_reclamo"];
		...
		$sql = "SELECT netbook_codigo FROM netbooks WHERE netbook_id = $netbook_id";		
		$consulta = consulta($sql,$conexion);
		$resultado = resultado($consulta);
		$netbook = $resultado["netbook_codigo"];
	}
} else {
	...
	if ($netbook_id != 0) {
	$sql = "SELECT soporte_id FROM soporte WHERE soporte_netbook_id = '$netbook_id' AND soporte_id != '$id' LIMIT 0,1";
	$consulta = consulta($sql,$conexion);
	...
	if ($caso != "") {
	$sql = "SELECT soporte_id FROM soporte WHERE soporte_nro_caso = '$caso' AND soporte_id != '$id' LIMIT 0,1";
	$consulta = consulta($sql,$conexion);
	...
	if ($msj == 0) {
		...
		//convertir fecha a segundos transcurridos 
		$descripcion = addslashes($descripcion);
		$reclamo = addslashes($reclamo);
		$sql = "UPDATE soporte SET soporte_nro_caso = '$caso',soporte_netbook_id='$netbook_id',soporte_estado_id='$estado_id',soporte_fecha_solicitud='$timestamp2',soporte_descripcion='$descripcion',soporte_reclamo='$reclamo' WHERE soporte_id = {$id}";
		...
		header ("Location: index.php?msj=62");
		exit;
	}
}
?>
...
<div id="centro">
	...
	<div class="cuadro-lista">
    	<div class="cuadro-encabezado">
        	<div class="cuadro-titulo">Editar solicitud de soporte</div>
        </div><!-- fin cuadro encabezado --> 
        <form name="cargar" method="POST" autocomplete="off" action="editar.php?estado=<?php echo $estado_listado;?>" onsubmit="return validarsoporte();">
